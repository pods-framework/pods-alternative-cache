...
class Pods_Alternative_Cache_Memcached extends Pods_Alternative_Cache_Storage {
    ...
    public function set_value( $cache_key, $cache_value, $expires = 0, $group = '' ) {
        ...
        $expires_at = ( 0 < (int) $expires ) ? time() + (int) $expires : 0;
    ...
    public function clear() {
        ...
        foreach ($keys as $index => $key) {
            ...
            if (strpos($key, $this->_namespace) !== false) {
    ...
    private function _connect()
    {
        ...
        $port   = (defined(PODS_ALT_CACHE_MEMCACHED_PORT))   ? PODS_ALT_CACHE_MEMCACHED_PORT   : "11211";
        $server = (defined(PODS_ALT_CACHE_MEMCACHED_SERVER)) ? PODS_ALT_CACHE_MEMCACHED_SERVER : "localhost" ;

        $this->_memcache->addServer($server, $port) or die ("Could not connect to memcache server");
    }
